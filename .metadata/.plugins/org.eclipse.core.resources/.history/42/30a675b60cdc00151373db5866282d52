...
public class BTreeFile implements DbFile {
	...
	private BTreeLeafPage findLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
		...
		// I'm on one particular page. It should have entries with fields so I can compare it...
		// iterate over internal records, find the right child pointer
		
		if(pid.pgcateg() == BTreePageId.LEAF){
			// we've reached a leaf node. In that case, get the page and return it
			...
			// if the field is null, find the left-most field get the first child and return it (this is dumb)
			if(f == null){
				BTreePageId childPageId = currPage.getChildId(0);
				return findLeafPage(tid,dirtypages,childPageId,perm,f);
			}else{
				// this is the body of the function
				
				Iterator<BTreeEntry> entIt = currPage.iterator();
				BTreeEntry currEnt;
				Field currKey;
				BTreePageId last = null;
				boolean gt;
				while(entIt.hasNext()){
					// find the correct key by looping
					currEnt = entIt.next();
					currKey = currEnt.getKey();
					gt = currKey.compare(Op.GREATER_THAN_OR_EQ, f);
					if(gt){ // if the current key is greater than the desired field, return the left child
						return findLeafPage(tid, dirtypages, currEnt.getLeftChild(), perm, f);
					}else{
						// if you haven't gone past it, store the right child
						// in case this was the last one, in which case you should return it
						// otherwise it'll be the left child of the next one
						last = currEnt.getRightChild();
					}
				}
				return findLeafPage(tid, dirtypages, last, perm, f);
			}
		}
	}
	...
	protected BTreeLeafPage splitLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page, Field field) 
		...
		// get the parent and make sure it has space
		
		BTreePageId myParent = page.getParentId();
		BTreeInternalPage openParent = this.getParentWithEmptySlots(tid, dirtypages, myParent, field);
		
		// get the old page and the new page
		
		BTreeLeafPage oldPage = page;
		BTreeLeafPage newPage = ((BTreeLeafPage) this.getEmptyPage(tid, dirtypages, BTreePageId.LEAF));
		
		// initialize the size of the page and the halfway point
		
		int halfway = oldPage.getMaxTuples()/2;
		int pageIndex = 0;
		
		// acquire iterator over tuples, use it
		// iterator starts at the end and goes backward - the first "halfway" entries are the ones
		// we want to cut and paste into the new page
		Iterator<Tuple> tupIt = oldPage.reverseIterator();
		Tuple currTup = null;
		
		while(tupIt.hasNext() && pageIndex < halfway){
			currTup = tupIt.next();
			oldPage.deleteTuple(currTup);
			newPage.insertTuple(currTup);
			pageIndex++;
		}
		
		// this is the last tuple that's copied over to the new pages
		// it's field will be the one that the new parent entry points to
		// I want to get the field of it that we're sorting on
		// I hope it's the first but not sure
		Field newKeyField = currTup.getField(0);
		
		// update sibling pointers
		
		newPage.setRightSiblingId(oldPage.getRightSiblingId());
		newPage.setLeftSiblingId(oldPage.getId());
		oldPage.setRightSiblingId(newPage.getId());
		
		// put a pointer to the new sibling into the parent page
			
			// first, make the new entry
			// find the key, then make a new entry
			// the key should be greater than or equal to the largest entry of oldpage
			// and less than or equal to the smallest entry of newpage
		
		BTreeEntry newEnt = new BTreeEntry(newKeyField, oldPage.getId(), newPage.getId());
		
			// next, insert this new entry into the parent page
		
		openParent.insertEntry(newEnt);
		
		// update the pointers back to the parent
		
		this.updateParentPointers(tid, dirtypages, openParent);
		
		// add dirtied pages to dirtypages
		
		dirtypages.put(openParent.getId(), openParent);
		dirtypages.put(oldPage.getId(), oldPage);
		dirtypages.put(newPage.getId(), newPage);
		
		// find which is the correct page and return it with the previous function
		
        return this.findLeafPage(tid, dirtypages, openParent.getId(), Permissions.READ_WRITE, field);
	...
	protected BTreeInternalPage splitInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		// first, get the parent and make sure that the parent has at least one slot to put the new data in
		// openParent should deal with the case that we're actually splitting the root
				
		BTreePageId myParent = page.getParentId();
		BTreeInternalPage openParent = this.getParentWithEmptySlots(tid, dirtypages, myParent, field);

		// get the old page and make a new page
		...
		int halfway = oldPage.getMaxEntries()/2;
		int pageIndex = 0;
		
		Iterator<BTreeEntry> entIt = oldPage.reverseIterator();
		BTreeEntry currEntry = null;
		
		while(entIt.hasNext() && pageIndex < halfway){
			currEntry = entIt.next();
			oldPage.deleteKeyAndRightChild(currEntry);
			newPage.insertEntry(currEntry);
			pageIndex++;
		}
		
		// currEntry is the last entry copied into the new page - the lowest value on the new page
		// need to decide whether to push up currEntry or the subsequent one
		// if the new field will become an entry in the new page, should push this
		// if the new field will become an entry in the old page, should push the next one
		
		Field middleField = currEntry.getKey();
		
		BTreeEntry pushEnt;
		
		boolean rightSpace = field.compare(Op.GREATER_THAN, middleField);
		if(rightSpace){
			// if the field to be inserted is greater than the current key,
			// it's going to be in the right sibling and we should definitely push the middle key
			newPage.deleteKeyAndLeftChild(currEntry);
			
			// in order to push an entry, make a new entry 
			// the field should be the field from the current entry
			// the left and right pointers should point to old and new pages respectively
			
			pushEnt = new BTreeEntry(currEntry.getKey(),oldPage.getId(), newPage.getId());
			this.pushEntry(pushEnt, openParent);
		}else{
			// if it's less, definitely push the other key
			// if they are the same, also push the other key:
				// if we pushed this key, then it would follow the left path to insert, which would
				// be wrong, and it would try to split again
			if(entIt.hasNext()){
				currEntry = entIt.next();
				oldPage.deleteKeyAndRightChild(currEntry);
				
				// see above for how pushing works
				
				pushEnt = new BTreeEntry(currEntry.getKey(),oldPage.getId(), newPage.getId());
				this.pushEntry(pushEnt, openParent);
			}else{
				throw new DbException("Tried to get the left halfway entry and couldn't find it");
			}
		}
		
		// no need to update siblings, not a thing for internal pages
	...
	private void pushEntry(BTreeEntry entry, BTreeInternalPage parent) throws DbException{
		
		// first, check that it's valid to insert
		
		//if(parent.getNumEmptySlots() == parent.getMaxEntries()){
			// if the parent is empty, insert it - it should work
	...
	protected void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
		...
		int halfFull = totalTuples/2;		
		
		if(isRightSibling){
			tupIt = sibling.iterator();
		}else{
			tupIt = sibling.reverseIterator();
		}
		
		Tuple currTuple = tupIt.next();
		
		while(page.getNumTuples() < halfFull){

			sibling.deleteTuple(currTuple);
			page.insertTuple(currTuple);
			if(!isRightSibling){
				entry.setKey(currTuple.getField(0));
				parent.updateEntry(entry);
			}
			currTuple = tupIt.next();
			if(isRightSibling){
				entry.setKey(currTuple.getField(0));
				parent.updateEntry(entry);
			}
	...
	protected void stealFromLeftInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		Iterator <BTreeEntry> entIt;
		
		int totalEntries = page.getNumEntries() + leftSibling.getNumEntries();
		int halfFull = totalEntries/2;	
		
		entIt = leftSibling.reverseIterator();
		
		BTreeEntry currEnt;
		Field moveKey;
		Field parentKey;
		
		while(page.getNumEntries() < halfFull){

			currEnt = entIt.next();
			
			moveKey = currEnt.getKey();
			
			parentKey = leftEntry.getKey();
			
			leftSibling.deleteKeyAndRightChild(currEnt);
			currEnt.setKey(parentKey);
		
			currEnt.setLeftChild(currEnt.getRightChild());
			currEnt.setRightChild(page.getChildId(0));
			
			page.insertEntry(currEnt);
			
			leftEntry.setKey(moveKey);
	...
	protected void stealFromRightInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		while(page.getNumEntries() < halfFull){
			...
			parentKey = rightEntry.getKey();
			
			rightSibling.deleteKeyAndLeftChild(currEnt);
			currEnt.setKey(parentKey);
		
			currEnt.setRightChild(currEnt.getLeftChild());
			currEnt.setLeftChild(page.reverseIterator().next().getRightChild());
			
			page.insertEntry(currEnt);
			
			rightEntry.setKey(moveKey);
